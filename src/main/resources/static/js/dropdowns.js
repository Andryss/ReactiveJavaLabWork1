...
async function loadSpaceshipsForDropdown(page = 0) {
    try {
        ...
        if (data.length === 0) {
            menu.innerHTML = '<li><div class="px-3 py-2 text-muted">No spaceships found</div></li>';
        ...
        data.forEach(ship => {
            ...
            a.href = '#';
            a.textContent = `${ship.serial} - ${ship.name || 'Unnamed'} (${ship.manufacturer || 'Unknown'})`;
            a.onclick = (e) => {
                e.preventDefault();
                document.getElementById('requestSpaceshipSerial').value = ship.serial;
                document.getElementById('spaceshipSelectedText').textContent = `${ship.serial} - ${ship.name || 'Unnamed'}`;
        ...
        if (selectedSerial) {
            ...
            if (selectedShip) {
                document.getElementById('spaceshipSelectedText').textContent = 
                    `${selectedShip.serial} - ${selectedShip.name || 'Unnamed'}`;
            }
        }
    } catch (error) {
        console.error('Error loading spaceships:', error);
        document.getElementById('spaceshipDropdownMenu').innerHTML = 
            '<li><div class="px-3 py-2 text-danger">Error loading spaceships</div></li>';
    }
}
...
function updateSpaceshipPaginationButtons() {
    ...
    pageInfo.textContent = `Page ${spaceshipPage + 1}`;
}
...
async function loadRepairmenForDropdown(page = 0) {
    try {
        ...
        if (data.length === 0) {
            menu.innerHTML = '<li><div class="px-3 py-2 text-muted">No repairmen found</div></li>';
        ...
        document.getElementById('assigneeDropdownMenu').innerHTML = 
            '<li><div class="px-3 py-2 text-danger">Error loading repairmen</div></li>';
    }
}
...
function updateAssigneePaginationButtons() {
    ...
    pageInfo.textContent = `Page ${assigneePage + 1}`;
}
