...
public class SpliteratorConcurrentStreamSpaceShipStatistics extends StreamSpaceShipStatistics {
	public SpliteratorConcurrentStreamSpaceShipStatistics(long delay) {
		super(delay);
	}

	public SpliteratorConcurrentStreamSpaceShipStatistics() {
		super();
	}
	...
	public OverallStatistics calculate(List<SpaceShip> objects) {
		...
		accumulator.getAggregateMaxSpeed().combine(
				StreamSupport.stream(new SpaceShipSpliterator(objects), true).mapToLong(SpaceShip::getMaxSpeed).summaryStatistics()
		);

		accumulator.getAggregateCrewMembers().combine(
				StreamSupport.stream(new SpaceShipSpliterator(objects), true).mapToLong(v -> v.getCrew().size()).summaryStatistics()
		);

		accumulator.getAggregateLength().combine(
				StreamSupport.stream(new SpaceShipSpliterator(objects), true).mapToLong(v -> v.getDimensions().length()).summaryStatistics()
		);
